...
// this class requires a VerticalLayoutGroup on the GameObject this is
...
public class UIUpgradeWindow : MonoBehaviour
{
    ...
    // default sprite used in the upgrade catalyst
    public Sprite defaultsprite;

    [Header("Settings")]
    public int maxOptions = 4; // We cannot show more upgrade options than this.
    ...
    public string evoIconPath = "Icon/Item Icon";
    ...
    public void SetUpgrades(PlayerInventory inventory, List<ItemData> possibleUpgrades, int pick = 3, string tooltip = "")
    {
        ...
        foreach (RectTransform r in upgradeOptions)
        {
            if (activeOptions < pick && activeOptions < totalPossibleUpgrades)
            {
                ...
                Image upgradeIcon = r.Find(evoIconPath).GetComponent<Image>();
                upgradeIcon.sprite = defaultsprite;
                if (upgradeIcon)
                {
                    Debug.Log("inupgradeicon");
                    if(selected.evolutionData.Length > 0)
                    {
                        Debug.Log(selected.evolutionData[0]);
                        Debug.Log(selected.evolutionData[0].catalysts[0]);
                        Debug.Log(selected.evolutionData[0].catalysts[0].itemType.icon);
                        upgradeIcon.sprite = selected.evolutionData[0].catalysts[0].itemType.icon;
                    }
