...
public class PlayerStats : MonoBehaviour
{
    ...
    void Awake()
    {
        characterData = GameDataManager.instance.GetSelectedCharacter();
        ...
        bonusSpawner = FindAnyObjectByType<BonusSpawner>();

        CharacterSaveDataObj charSaveData = ES3.Load<CharacterSaveDataObj>(GameDataManager.instance.GetSelectedCharacter().Name);
        Debug.Log(charSaveData.CharLevel);
        ...
        if(charSaveData != null)
        {
            ...
            float dmgboost = charSaveData.CharLevel;
            dmgboost = dmgboost / 10;

            float healthBoost = charSaveData.CharLevel;
            healthBoost = healthBoost / 10;

      
            baseStats.might += dmgboost;
            actualStats.might += dmgboost;
            baseStats.maxHealth += healthBoost;
            actualStats.maxHealth += healthBoost;
        }
    ...
    {
        Debug.Log(collector + "radius set 500");
        collector.SetRadius(500);
        Invoke(nameof(CollectOff), 0.2f);
    }
    public void CollectOff()
    {
        Debug.Log(collector + "radius set back to original");
        collector.SetRadius(actualStats.magnet);
    }
    ...
    public void Kill()
    {
        if (!GameManager.instance.isGameOver && !bonusSpawner.RoundEnded)
        {
            GameManager.instance.AssignLevelReachedUI(level);
            GameManager.instance.AssignChosenWeaponsAndPassiveItemsUI(inventory.weaponSlots, inventory.passiveSlots);
            GameManager.instance.GameOver();
        }else if(!GameManager.instance.isGameOver && bonusSpawner.RoundEnded)
        {
            GameManager.instance.ChangeState(GameManager.GameState.LevelDone);
        }
    }
