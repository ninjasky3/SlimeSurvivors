...
public class EnemySpawner : MonoBehaviour
{
    ...
    [Header("UI Display")]
    public TMP_Text enemyTextDisplay;
    ...
    void Update()
    {
        if (currentWaveCount < waves.Count && waves[currentWaveCount].spawnCount == waves[currentWaveCount].waveQuota && !isWaveActive)  //Check if the wave has ended and the next wave should start
        {
    ...
    IEnumerator BeginNextWave()
    {
        ...
        {
            player.CollectAll();
            Invoke(nameof(StartBonus), 3);
        }
    }
    void StartBonus()
    {
        bonusSpawner.RoundEnded = true;
    }
    void CalculateWaveQuota()
    {
        ...
        enemiesKilled += currentWaveQuota;
        //set display text to wave quota.
        enemyTextDisplay.text = enemiesKilled.ToString();
    ...
    public void OnEnemyKilled()
    {
        ...
        //enemiesKilled to take off the enemy display
        enemiesKilled--;
