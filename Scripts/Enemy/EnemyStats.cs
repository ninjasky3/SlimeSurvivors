...
public class EnemyStats : MonoBehaviour
{
    ...
    public void Kill()
    {
        Collider2D rb = GetComponent<Collider2D>();
        rb.enabled = false;
        StartCoroutine(KillFade());

        player.getGold(goldWorth);
    }
    ...
    {
        BonusSpawner bs = FindObjectOfType<BonusSpawner>();
        if (bs && bs.RoundEnded) bs.OnEnemyKilled();
        EnemySpawner es = FindObjectOfType<EnemySpawner>();
        if(es && !bs.RoundEnded) es.OnEnemyKilled();
    }

    void ReturnEnemy()
    {
        EnemySpawner es = FindObjectOfType<EnemySpawner>();
        transform.position = playerTransform.position + es.relativeSpawnPoints[Random.Range(0, es.relativeSpawnPoints.Count)].position;
    }
}
